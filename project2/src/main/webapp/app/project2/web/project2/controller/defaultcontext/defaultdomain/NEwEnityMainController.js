Ext.define('Project2.project2.web.project2.controller.defaultcontext.defaultdomain.NEwEnityMainController', {
     extend: 'Project2.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NEwEnityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NEwEnityTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#NEwEnityForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NEwEnityGrid');
               var tree = this.view.down('#NEwEnityTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#NEwEnityForm');
          var grid = this.view.down('#NEwEnityGrid');
          var tree = this.view.down('#NEwEnityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NEwEnityGrid');
               ...
               var formPanel = this.getView().up().down('#NEwEnityForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NEwEnityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#NEwEnityForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NEwEnityGrid').store;
          gridStore.removeAll();
