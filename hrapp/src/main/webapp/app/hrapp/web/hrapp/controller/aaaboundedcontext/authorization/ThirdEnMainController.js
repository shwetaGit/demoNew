Ext.define('Hrapp.hrapp.web.hrapp.controller.aaaboundedcontext.authorization.ThirdEnMainController', {
     extend: 'Hrapp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ThirdEnMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ThirdEnTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#ThirdEnForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#ThirdEnGrid');
               var tree = this.view.down('#ThirdEnTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#ThirdEnForm');
          var grid = this.view.down('#ThirdEnGrid');
          var tree = this.view.down('#ThirdEnTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#ThirdEnGrid');
               ...
               var formPanel = this.getView().up().down('#ThirdEnForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#ThirdEnTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#ThirdEnForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ThirdEnGrid').store;
          gridStore.removeAll();
