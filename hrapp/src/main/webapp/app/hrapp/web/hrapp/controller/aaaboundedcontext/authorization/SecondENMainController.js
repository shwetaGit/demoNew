Ext.define('Hrapp.hrapp.web.hrapp.controller.aaaboundedcontext.authorization.SecondENMainController', {
     extend: 'Hrapp.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.SecondENMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#SecondENTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#SecondENForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#SecondENGrid');
               var tree = this.view.down('#SecondENTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#SecondENForm');
          var grid = this.view.down('#SecondENGrid');
          var tree = this.view.down('#SecondENTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#SecondENGrid');
               ...
               var formPanel = this.getView().up().down('#SecondENForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#SecondENTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#SecondENForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#SecondENGrid').store;
          gridStore.removeAll();
