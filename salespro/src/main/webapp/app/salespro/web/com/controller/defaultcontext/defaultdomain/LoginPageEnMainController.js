Ext.define('Salespro.salespro.web.com.controller.defaultcontext.defaultdomain.LoginPageEnMainController', {
     extend: 'Salespro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.LoginPageEnMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#LoginPageEnTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#LoginPageEnForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#LoginPageEnGrid');
               var tree = this.view.down('#LoginPageEnTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#LoginPageEnForm');
          var grid = this.view.down('#LoginPageEnGrid');
          var tree = this.view.down('#LoginPageEnTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#LoginPageEnGrid');
               ...
               var formPanel = this.getView().up().down('#LoginPageEnForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#LoginPageEnTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#LoginPageEnForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#LoginPageEnGrid').store;
          gridStore.removeAll();
