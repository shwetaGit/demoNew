Ext.define('Oct16last.oct16last.web.oct16last.controller.defaultdomain.CheckboxTestMainController', {
     extend: 'Oct16last.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CheckboxTestMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CheckboxTestTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#CheckboxTest');
          var grid = this.view.down('#CheckboxTestGrid');
          var tree = this.view.down('#CheckboxTestTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#CheckboxTestGrid');
               ...
               var formPanel = this.getView().up().down("#CheckboxTest");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#CheckboxTestTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CheckboxTest').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CheckboxTestGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.pk;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#CheckboxTest').getViewModel().setData(responseData.response.data);
     },
