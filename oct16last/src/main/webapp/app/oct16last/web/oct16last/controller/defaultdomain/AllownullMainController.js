Ext.define('Oct16last.oct16last.web.oct16last.controller.defaultdomain.AllownullMainController', {
     extend: 'Oct16last.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.AllownullMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#AllownullTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Allownull');
          var grid = this.view.down('#AllownullGrid');
          var tree = this.view.down('#AllownullTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#AllownullGrid');
               ...
               var formPanel = this.getView().up().down("#Allownull");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#AllownullTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Allownull').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#AllownullGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.pk1;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Allownull').getViewModel().setData(responseData.response.data);
     },
