Ext.define('Oct16last.oct16last.web.oct16last.controller.defaultdomain.TestingMainController', {
     extend: 'Oct16last.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestingMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestingTree');
     ...
     hideDataBrowser: function() {
          debugger;
          var form = this.view.down('#Testing');
          var grid = this.view.down('#TestingGrid');
          var tree = this.view.down('#TestingTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#TestingGrid');
               ...
               var formPanel = this.getView().up().down("#Testing");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          var treePanel = this.view.up().up().up().down('#TestingTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Testing').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestingGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#Testing').getViewModel().setData(responseData.response.data);
     },
