package com.basehr.app.server.businessservice.aaaboundedcontext.authentication.aspect;
...
public class BusinessServiceauthenticationAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.basehr.app.server.businessservice.aaaboundedcontext.authentication..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("ABSAN217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("ABSAN227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            BusinessException businessException = new BusinessException();
            businessException.getException("ABS", "AN", e, runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint);
        } finally {
