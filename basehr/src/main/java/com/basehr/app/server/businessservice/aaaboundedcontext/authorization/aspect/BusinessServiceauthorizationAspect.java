package com.basehr.app.server.businessservice.aaaboundedcontext.authorization.aspect;
...
public class BusinessServiceauthorizationAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.basehr.app.server.businessservice.aaaboundedcontext.authorization..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("ABSAU217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("ABSAU227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            BusinessException businessException = new BusinessException();
            businessException.getException("ABS", "AU", e, runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint);
        } finally {
