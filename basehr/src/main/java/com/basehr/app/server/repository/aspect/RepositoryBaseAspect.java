package com.basehr.app.server.repository.aspect;import java.util.HashMap;
...
public class RepositoryBaseAspect extends RepositoryAspect{
	...
	@Pointcut("execution(* com.spartan..repository..*(..))) && !within(com.athena.server.query.repository.ArtQueryRepository+) && ! cloudOperation() && !within(com.spartan..alarms..*)")
	protected void applifireRepositoryOperation() {
	}

	@Pointcut("execution(* com.athena.server.query.repository.ArtQueryRepository.findSqlFromJPQL())")
