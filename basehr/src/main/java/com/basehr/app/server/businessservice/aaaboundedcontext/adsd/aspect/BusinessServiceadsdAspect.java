package com.basehr.app.server.businessservice.aaaboundedcontext.adsd.aspect;
import com.basehr.app.server.businessservice.aspect.BusinessAspect;
...
import com.basehr.app.server.businessservice.aspect.BusinessException;
...
@Aspect
@Component
public class BusinessServiceadsdAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.basehr.app.server.businessservice.aaaboundedcontext.adsd..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("ABSHE217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("ABSHE227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            BusinessException businessException = new BusinessException();
            businessException.getException("ABS", "HE", e, runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint);
        } finally {
