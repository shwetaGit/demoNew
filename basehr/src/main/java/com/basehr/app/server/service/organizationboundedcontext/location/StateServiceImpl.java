...
public class StateServiceImpl extends StateService {
    ...
    public HttpEntity<ResponseBean> findAll() throws Exception {
        java.util.List<com.basehr.app.shared.organizationboundedcontext.location.State> lststate = staterepo.findAll();
    ...
    public HttpEntity<ResponseBean> findByCountryId(@RequestBody FindByBean findByBean) throws Exception {
        ...
        List<com.basehr.app.shared.organizationboundedcontext.location.State> lststate = staterepo.findByCountryId((java.lang.String) findByBean.getFindKey());
        ...
        responseBean.add("message", String.format(appAlarm.getMessage(), "State"));
        responseBean.add("data", lststate);
        Log.out.println("ORGLO124100200", runtimeLogInfoHelper.getRequestHeaderBean(), "StateServiceImpl", "save", "State");
        return new org.springframework.http.ResponseEntity<ResponseBean>(responseBean, HttpStatus.valueOf(appAlarm.getAlarmStatus()));
    }
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws Exception {
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        com.basehr.app.shared.organizationboundedcontext.location.State lststate = staterepo.findById((java.lang.String) findByBean.getFindKey());
        AppAlarm appAlarm = Log.getAlarm("null");
        com.athena.framework.server.bean.ResponseBean responseBean = new ResponseBean(appAlarm);
        responseBean.add("message", String.format(appAlarm.getMessage(), "State"));
        responseBean.add("data", lststate);
        Log.out.println("null", runtimeLogInfoHelper.getRequestHeaderBean(), "StateServiceImpl", "save", "State");
        return new org.springframework.http.ResponseEntity<ResponseBean>(responseBean, HttpStatus.valueOf(appAlarm.getAlarmStatus()));
    }
}
