package com.basehr.app.server.businessservice.organizationboundedcontext.contacts.aspect;
...
public class BusinessServicecontactsAspect extends BusinessAspect {
    ...
    @Pointcut("execution(* com.basehr.app.server.businessservice.organizationboundedcontext.contacts..*(..))")
    ...
    public Object aroundfindAll(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Object object = null;
        try {
            Log.out.println("ORGCN217100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            ...
            Log.out.println("ORGCN227100100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName());
        } catch (Exception e) {
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_EXCEPTION, HealthConstants.DEFAULT_EXCEPTION_ID);
            BusinessException businessException = new BusinessException();
            businessException.getException("ORG", "CN", e, runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint);
        } finally {
