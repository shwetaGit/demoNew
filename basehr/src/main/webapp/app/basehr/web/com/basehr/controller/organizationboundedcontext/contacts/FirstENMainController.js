Ext.define('Basehr.basehr.web.com.basehr.controller.organizationboundedcontext.contacts.FirstENMainController', {
     extend: 'Basehr.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.FirstENMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#FirstENTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#FirstENForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#FirstENGrid');
               var tree = this.view.down('#FirstENTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#FirstENForm');
          var grid = this.view.down('#FirstENGrid');
          var tree = this.view.down('#FirstENTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#FirstENGrid');
               ...
               var formPanel = this.getView().up().down('#FirstENForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#FirstENTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#FirstENForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#FirstENGrid').store;
          gridStore.removeAll();
