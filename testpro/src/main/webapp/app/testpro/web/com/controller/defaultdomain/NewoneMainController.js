Ext.define('Testpro.testpro.web.com.controller.defaultdomain.NewoneMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NewoneMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NewoneTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#NewoneGrid');
               grid.store.add(data);
               var tree = this.view.down('#NewoneTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Newone');
          var grid = this.view.down('#NewoneGrid');
          var tree = this.view.down('#NewoneTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#NewoneGrid');
               ...
               var formPanel = this.getView().up().down("#Newone");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#NewoneTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Newone').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NewoneGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#Newone').getViewModel().setData(responseData.response.data);
     },
