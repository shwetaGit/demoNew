Ext.define('Testpro.testpro.web.com.controller.defaultdomain.TestEntityMainController', {
     extend: 'Testpro.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestEntityMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestEntityTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#TestEntityGrid');
               grid.store.add(data);
               var tree = this.view.down('#TestEntityTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestEntity');
          var grid = this.view.down('#TestEntityGrid');
          var tree = this.view.down('#TestEntityTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#TestEntityGrid');
               ...
               var formPanel = this.getView().up().down("#TestEntity");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#TestEntityTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestEntity').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestEntityGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#TestEntity').getViewModel().setData(responseData.response.data);
     },
