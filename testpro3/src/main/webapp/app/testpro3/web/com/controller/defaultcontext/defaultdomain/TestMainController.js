Ext.define('Testpro3.testpro3.web.com.controller.defaultcontext.defaultdomain.TestMainController', {
     extend: 'Testpro3.view.fw.frameworkController.AggregateViewController',
     alias: 'controller.TestMainController',
     init: function() {
     ...
     renderTreeGridData: function(responseData) {
          ...
          if (responseData.response.success) {
               ...
               var tree = this.view.down('#TestTree');
               ...
               var compRef = this.view.down('#TestTree');
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.tid;
          me = this;
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Test');
          var grid = this.view.down('#TestGrid');
          var tree = this.view.down('#TestTreeContainer');
     ...
     onFilterClick: function(but, evt) {
          ...
          Ext.Ajax.request({
               ...
               failure: function(response, currentObject) {
                    ...
                    var compRef = scope.sender.view.down('#TestTree');
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var Test = this.view.down('#TestForm');
               Test.reset();
               var gridPanel = this.view.down('#TestGrid');
          ...
          var formPanel = this.getView().up().down('#TestForm');
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#TestTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Test').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestGrid').store;
          gridStore.removeAll();
          gridStore.add(data);
     ...
     loadData: function(responseData) {
          this.view.down('#TestForm').getViewModel().setData(responseData.response.data);
     },
