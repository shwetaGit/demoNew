Ext.define('Testprj.testprj.web.testprj.controller.defaultdomain.PersonalInfoMainController', {
     extend: 'Testprj.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.PersonalInfoMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#PersonalInfoTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#PersonalInfoGrid');
               grid.store.add(data);
               var tree = this.view.down('#PersonalInfoTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#PersonalInfo');
          var grid = this.view.down('#PersonalInfoGrid');
          var tree = this.view.down('#PersonalInfoTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#PersonalInfoGrid');
               ...
               var formPanel = this.getView().up().down("#PersonalInfo");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#PersonalInfoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#PersonalInfo').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#PersonalInfoGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#PersonalInfo').getViewModel().setData(responseData.response.data);
     },
