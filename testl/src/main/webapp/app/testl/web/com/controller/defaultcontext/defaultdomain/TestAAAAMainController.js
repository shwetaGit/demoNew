Ext.define('Testl.testl.web.com.controller.defaultcontext.defaultdomain.TestAAAAMainController', {
     extend: 'Testl.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestAAAAMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestAAAATree');
     ...
     refreshMainForm: function(but, data, method) {
          ...
          var formPanel = this.view.down('#TestAAAA');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestAAAAGrid');
               var tree = this.view.down('#TestAAAATree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestAAAA');
          var grid = this.view.down('#TestAAAAGrid');
          var tree = this.view.down('#TestAAAATreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestAAAAGrid');
               ...
               var formPanel = this.getView().up().down('#TestAAAA');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestAAAATree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestAAAA').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestAAAAGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#TestAAAA').getViewModel().setData(responseData.response.data);
     },
