Ext.define('Testl.testl.web.com.controller.defaultcontext.defaultdomain.NewrrrMainController', {
     extend: 'Testl.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.NewrrrMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#NewrrrTree');
     ...
     refreshMainForm: function(but, data, method) {
          ...
          var formPanel = this.view.down('#Newrrr');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#NewrrrGrid');
               var tree = this.view.down('#NewrrrTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Newrrr');
          var grid = this.view.down('#NewrrrGrid');
          var tree = this.view.down('#NewrrrTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#NewrrrGrid');
               ...
               var formPanel = this.getView().up().down('#Newrrr');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#NewrrrTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Newrrr').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#NewrrrGrid').store;
          gridStore.removeAll();
     ...
     loadData: function(responseData) {
          this.view.down('#Newrrr').getViewModel().setData(responseData.response.data);
     },
