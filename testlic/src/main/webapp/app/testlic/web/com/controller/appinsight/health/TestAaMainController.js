Ext.define('Testlic.testlic.web.com.controller.appinsight.health.TestAaMainController', {
     extend: 'Testlic.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.TestAaMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#TestAaTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#TestAaForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#TestAaGrid');
               var tree = this.view.down('#TestAaTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#TestAaForm');
          var grid = this.view.down('#TestAaGrid');
          var tree = this.view.down('#TestAaTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#TestAaGrid');
               ...
               var formPanel = this.getView().up().down('#TestAaForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#TestAaTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#TestAaForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#TestAaGrid').store;
          gridStore.removeAll();
